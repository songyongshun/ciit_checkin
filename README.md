...
3. 启动服务：
    ...
    部署到服务器、需外部访问时，用 `0.0.0.0` 监听所有网卡（并确保防火墙放行端口）：
      ```bash
      checkin -c room_info.yaml --port 8000 --host 0.0.0.0
      ```
    - 二维码/管理页里使用的地址，可单独在 `room_info.yaml` 中配置 `public_ip`（填公网 IP 或域名），
      这样即使监听地址是 `0.0.0.0`，扫码也能正确打开签到页。未配置时回退为监听地址。
...
- 监听地址：由 `--host` 参数控制，默认 `127.0.0.1`（仅本机可访问）。
- `public_ip`（可选）：用于二维码和管理页展示的对外地址，可在配置文件中用 `public_ip: "1.2.3.4"` 或域名显式指定；未指定时回退为 `--host` 的值。
- 配置文件（示例路径，可根据代码调整）：
  - data/name.txt — 名单
